...
const WebcamSurveillance = dynamic(() => import('@/components/WebcamSurveillance'), {
    ssr: false
});
...
export default function StudentTest() {
    ...
    const initializeTest = async () => {
        try {
            ...
            // Fetch questions
            const response = await fetch(`http://localhost:8000/questions/${student.field}`, {
                headers: {
    ...
    const analyzeCognitivePattern = (answer, responseTime, isCorrect) => {
        setCognitiveData(prev => {
            ...
            if (newHesitationTimes.length > 1) {
                const avgTime = newHesitationTimes.reduce((a, b) => a + b) / newHesitationTimes.length;
    ...
    const finishTest = async () => {
        try {
            ...
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/test/submit`, {
                method: 'POST',
                headers: {
    ...
    const handleSurveillanceAlert = (alertData) => {
        setAlertsHistory(prev => [...prev, { ...alertData, timestamp: Date.now() }]);
        
        // Update surveillance metrics based on alert type
        setSurveillanceMetrics(prev => ({
            ...prev,
            ...(alertData.type === 'position' && { positionViolations: prev.positionViolations + 1 }),
            ...(alertData.type === 'speech' && { speechDetections: prev.speechDetections + 1 }),
            ...(alertData.type === 'multiple_persons' && { multiplePersonsDetected: prev.multiplePersonsDetected + 1 }),
                ...
                <div className="max-w-4xl mx-auto">
                    <div className="bg-white rounded-2xl shadow-2xl p-8">
                        <div className="text-center mb-8">
                            ...
                            <p className="text-gray-600">Votre signature cognitive a été générée</p>
                        </div>
                        ...
                        <div className="bg-blue-50 rounded-xl p-6 mb-6">
                                ...
                                <div className="bg-white rounded-lg p-4">
                                    <div className="flex justify-between items-center mb-2">
                                        ...
                                        <span className="font-bold text-blue-600">
                                            {testResults?.cognitive_twin?.twin_id}
                                        </span>
                                    </div>
                                    <div className="flex justify-between items-center mb-2">
                                        <span className="text-gray-600">Type Cognitif:</span>
                                        <span className="font-bold text-green-600">
                                            {testResults?.cognitive_twin?.metrics?.cognitive_type}
                                        </span>
                                    </div>
                                    <div className="flex justify-between items-center mb-2">
                                        <span className="text-gray-600">Précision:</span>
                                        <span className="font-bold text-purple-600">
                                            {testResults?.accuracy}% ({testResults?.correct_answers}/{testResults?.total_questions})
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div className="bg-white rounded-lg p-4">
                                    <div className="flex justify-between items-center mb-2">
                                        ...
                                        <span className="font-bold text-orange-600">
                                            {testResults?.cognitive_twin?.metrics?.avg_response_time}ms
                                        </span>
                                    </div>
                                    <div className="flex justify-between items-center mb-2">
                                        <span className="text-gray-600">Niveau de Focus:</span>
                                        <span className="font-bold text-red-600">
                                            {testResults?.cognitive_twin?.metrics?.focus_level}/100
                                        </span>
                                    </div>
                                    <div className="flex justify-between items-center mb-2">
                            ...
                            <div className="mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                                <p className="text-sm text-yellow-800">
                                    <strong>📊 Signature stockée:</strong> Votre TWIN cognitif est maintenant enregistré 
                                    et peut être utilisé pour de futures authentifications ou analyses comparatives.
                                </p>
                            </div>
                        </div>
                ...
                <div className="bg-white rounded-2xl shadow-2xl p-6">
                    ...
                    <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-6">
                        <p className="text-sm text-green-800">
                            <strong>ℹ️ Information:</strong> Ce test mesure votre signature cognitive unique (TWIN). 
                            Répondez naturellement, nous analysons vos patterns de réponse.
                        </p>
                    </div>
